...
class Timer(object):
    ...
    def local_jd_time_diff(self):
        ...
        :return:
        """
        val = self.local_time() - self.jd_time()
        return val
    ...
    def start(self):
        ...
        while True:
            ...
            # 将日期和时间转换为时间元组  
            current_time_tuple = now_time().timestamp()  
            # 将时间元组转换为毫秒数  
            current_timestamp_milliseconds = int(current_time_tuple * 1000) 
            if current_timestamp_milliseconds >= self.buy_time_ms:
